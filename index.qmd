...
```{r}
# <- pdf_text("unl-department-budget-2010-2011-pt1.pdf")
meta_2010_1 <- pdf_info("unl-department-budget-2010-2011-pt1.pdf")
meta_2010_2 <- pdf_info("unl-department-budget-2010-2011-pt2.pdf")
meta_2015 <- pdf_info("unl-department-budget-2015-2016.pdf")
meta_2020 <- pdf_info("unl-department-budget-2020-2021.pdf")
meta_2025 <- pdf_info("unl-department-budget-2025-2026.pdf")

#meta_2010_1[["metadata"]]
#meta_2010_2[["metadata"]]
#meta_2015[["metadata"]]
#meta_2020[["metadata"]]
#meta_2025[["metadata"]]

meta_combined <- list(
    "2010_1" = meta_2010_1[["metadata"]],
    "2010_2" = meta_2010_2[["metadata"]],
    "2015" = meta_2015[["metadata"]],
    "2020" = meta_2020[["metadata"]],
    "2025" = meta_2025[["metadata"]]
)

df <- meta_combined %>%
    map_dfr(~ as.data.frame(.), .id="source_year")

df <- df %>%
    mutate(
        File = str_extract(`.`, "(?<=File=['\"])[^'\"]+"),
        Creator = str_extract(`.`, "(?<=Creator=['\"])[^'\"]+"),
        Author = str_extract(`.`, "(?<=Author=['\"])[^'\"]+"),
        Producer = str_extract(`.`, "(?<=Producer=['\"])[^'\"]+"),
...
clean("unl-department-budget-2010-2011-pt1.pdf")
```
------------------------------------------------------------------------

I decided to move on, but looked at the individual metadatas and found the info myself. The big thing I noticed was how different the 2025 budget is from the others. There was no title available and both the creator tool and producer were different from the others. Also, the modify dates are different but that should be obvious. Something that did stick out to me was the creation date being the same for 2015 and 2020, but I don't believe that would change how we go about extracting the pdf text. 
...
```{r}
# Don't know if I really have anything to put here?
```

------------------------------------------------------------------------

The differences above make me think the layout or exact positioning for the 2025 budget will be different and will require different coordinates and functions to scrape the data correctly. I think I'll be able to modify the code I used for the first four to get the last pdf, but maybe that's just wishful thinking.   
...
#}
#find_page_number("unl-department-budget-2010-2011-pt1.pdf")
...
find_page_number <- function(file_path) {
  text <- pdf_text(file_path)
  #doing this because it is so late and I'm losing my mind
  text <- text[-622] 
  
  keyword1 <- "salaries"
  keyword2 <- "operating"
  
  has_budget <- str_detect(text, regex(paste0("\\b", keyword1, "\\b"), ignore_case = TRUE))
  has_permanent <- str_detect(text, regex(paste0("\\b", keyword2, "\\b"), ignore_case = TRUE))
  
  salary_true <- has_budget & has_permanent
  
  if (any(salary_true)) {
    start_page <- which(salary_true)[1]
    end_page <- which(salary_true)[length(which(salary_true))]
  } else {
    start_page <- NA_integer_
    end_page <- NA_integer_
  }
...
find_page_number(("unl-department-budget-2010-2011-pt1.pdf"))
find_page_number(("unl-department-budget-2010-2011-pt2.pdf"))
